...
class _SearchModalState extends State<SearchModal> {
  ...
  void initState() {
    ...
    if (Config().useContactsLocation) {
      unawaited(ContactInfo.loadAll().then((success) {
        if (!success && mounted) {
          setState(() {
            _error = 'Contacts permission is denied';
          });
        }
      }));
    }
  ...
  void _loadContacts() {
    try {
      final filtered = ContactInfo.currentContacts.value
  ...
  void _loadStops() {
    _allStops = Stop.currentStops.value
        .map(
          (stop) => Location(
            name: stop.name,
  ...
  void _onTextChanged() {
    ...
    final filteredStops = _allStops
        ...
        .toList();
    final filteredFavourites = _favourites
